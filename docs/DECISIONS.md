...
| Adapter específico Nightly | **`@nightlylabs/wallet-selector-solana`** | 0.3.14 (checado via `npm view`) | Pacote oficial da Nightly para redes "Solana-like" — usado por múltiplas submissões de terceiro do próprio bounty (ver `docs/research/ECOSYSTEM.md` §4). A rede Cookie Chain é adicionada como rede SVM customizada via `genesisHash` + `url` (mecanismo genérico documentado pela Nightly, não uma integração dedicada anunciada — ver ressalva no mesmo documento). |

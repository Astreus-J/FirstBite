...
- **T6/T15 (bot farming / rate limit)**: dois limitadores em memória (`app/src/lib/rate-limit.ts`) — por `bite` (10 req/min) e por IP (30 req/min, extraído de `x-forwarded-for`/`x-real-ip`). Ambos com capacidade máxima de chaves rastreadas (5.000), com descarte das mais antigas quando excedida — ver correção do achado M2 abaixo. Ainda documentado como proteção básica, não durável entre deploys/instâncias.
...
## Segunda revisão de segurança independente (M6.2, 2026-09-14)

Revisão feita por um agente com contexto limpo (não o mesmo que escreveu o código), especificamente sobre o programa Anchor e o Sponsor Service. Nenhuma vulnerabilidade **Critical** encontrada — nenhum caminho para roubo direto de fundos, bypass de assinatura, ou drenagem instantânea do sponsor por instrução forjada. Achados relevantes:

| # | Severidade | Achado | Status |
|---|---|---|---|
| H1 | High | A tabela de T6 (acima) já descrevia rate limit "por IP e por bite_pubkey", mas a implementação inicial do M4.2 só tinha o limite por `bite` — nenhum limite agregado por IP. Um atacante podia criar N Bites pequenos autofinanciados (custo ≈ só a fee de `create_bite`, que ele mesmo paga) e reivindicar cada um com uma wallet nova, sem nenhum teto agregado sobre quantas fees o sponsor paga no total. | **Corrigido** — `ipRateLimiter` adicionado em `route.ts`, checado antes de qualquer trabalho on-chain. |
| M2 | Medium | `recentRequests` (o `Map` do rate limiter) nunca removia chaves, só filtrava os timestamps de cada uma. Como a checagem por `bite` acontece antes de confirmar que o Bite existe, um atacante gerando pubkeys aleatórias em loop crescia o `Map` indefinidamente — DoS de memória barato contra o processo do Sponsor Service. | **Corrigido** — `BoundedRateLimiter` agora limita a 5.000 chaves rastreadas, descartando as mais antigas quando excede. Testado em `app/src/lib/rate-limit.test.ts` (`node --experimental-strip-types --test`, 6/6 passando), incluindo um teste que prova o limite de 5.000 mesmo sob 6.000 chaves distintas. |
| M1 | Medium | TOCTOU entre a leitura do estado do Bite (`route.ts`) e a assinatura do sponsor: duas requisições concorrentes perto do limite de `max_claims` podem ambas passar na checagem off-chain, e uma delas falha on-chain (`BiteNotActive`) depois de já ter consumido uma assinatura/fee do sponsor. | **Risco aceito** — corrigir exigiria um lock/fila por Bite, desproporcional para o MVP; o custo do pior caso é o valor de uma fee de rede por corrida, próximo ao esgotamento natural do Bite. Documentado aqui em vez de deixar implícito. |
| M3 | (confirmação) | Nenhum teste automatizado cobria o Sponsor Service antes desta revisão. | **Parcialmente corrigido** — `rate-limit.ts` agora tem cobertura de teste; a rota HTTP completa (`route.ts`) continua sem teste de integração automatizado (Fase 17, item ainda em aberto). |
| L1–L3 | Low | `expiration` no passado não é bloqueado em `create_bite` (só prejudica o próprio criador); corrida `cancel_bite` vs. claim em voo (baixo valor para um atacante); constante de rent hardcoded no frontend é só validação de UX, a autoritativa é on-chain. | Aceitos como estão — impacto mínimo, correção desproporcional ao benefício para o MVP. |

Pontos confirmados como bem implementados pela revisão (não apenas aceitos por padrão): toda aritmética sensível do programa usa `checked_*`, nenhuma CPI arbitrária, PDAs sempre derivadas com `seeds`+`bump` (nunca vindo do cliente), `ClaimRecord` via `init` é uma barreira genuína contra claim duplicado, e o Sponsor Service nunca assina uma transação vinda do cliente nem loga a chave do sponsor.

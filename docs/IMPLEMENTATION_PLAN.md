...
| M5.4 | Landing page final + histórico do creator | `feature/transaction-feedback` | M3.2 | Homepage comunica o produto em ~5s | DONE — página `/history` (lista Bites do creator via `program.account.bite.all()` com filtro `memcmp`, com ação "Cancel & reclaim") + nav simples (Home/Create/Your Bites) em todas as páginas. Testado de ponta a ponta: criar → ver no histórico → cancelar → some da lista. **Atualização (2026-09-14, mesmo dia)**: revisão de design completa feita com a Skill `impeccable` — paleta OKLCH própria (âmbar + chocolate escuro, contraste WCAG calculado, não estimado), hero + sequência real de 3 passos na home, card do valor na tela de claim que muda de âmbar para verde ao confirmar, QR com botão de copiar, estados vazios/skeleton no histórico. Verificado em desktop e mobile, zero erros de console. Achados extras corrigidos no processo: mismatch de hidratação real no botão de wallet (`ssr: false` via wrapper) e um bug onde `/api/claim` podia devolver uma resposta não-JSON em erros inesperados. |

...
|---|---|---|---|---|---|---|
| M1.1 | Inicializar workspace Anchor (`anchor init`) | `feature/anchor-program` | M0.4 | `programs/`, `Anchor.toml`, `Cargo.toml` | `anchor build` roda sem erro | manual |
| M1.2 | Instrução `create_bite` | `feature/anchor-program` | M1.1 | `programs/first_bite/src/instructions/create_bite.rs` | Cria PDA `Bite` com seeds `[b"bite", creator, bite_id]`, valida `amount_per_claim >= 890_880`, transfere depósito total do creator para o PDA | `anchor test`: criação válida, amount abaixo do mínimo rejeitado, creator sem saldo suficiente rejeitado |
| M1.3 | Instrução `claim` | `feature/anchor-program` | M1.2 | `.../claim.rs` | Cria `ClaimRecord` PDA `[b"claim", bite, claimer]` (falha se já existir), valida `status == Active`, valida `expiration` se definido, valida `claimed_count < max_claims`, transfere `amount_per_claim` do PDA do Bite para o claimer, incrementa `claimed_count` (checked) | `anchor test`: claim válido, segundo claim do mesmo claimer rejeitado, Bite vazio rejeitado, Bite expirado rejeitado |
| M1.4 | Instrução `cancel_bite` | `feature/anchor-program` | M1.2 | `.../cancel_bite.rs` | Só o `creator` original pode cancelar (`has_one`); devolve `remaining_amount` e fecha a conta (`close = creator`) | `anchor test`: cancelamento pelo creator correto funciona, tentativa por outra wallet é rejeitada, saldo restante bate |
| M1.5 | Overflow/underflow safety pass | `feature/anchor-program` | M1.2–M1.4 | mesmos arquivos | Toda aritmética usa `checked_add`/`checked_sub`, sem `unwrap()` silencioso em caminho de produção | revisão manual + `solana-vulnerability-scanner` |
| M1.6 | Rodar `solana-vulnerability-scanner` e `entry-point-analyzer` sobre o programa | `feature/anchor-program` | M1.5 | — | Nenhum finding CONFIRMED sem mitigação documentada | Skill run |

...
| M4.4 | Fallback se a Nightly real não aceitar `feePayer` alternativo | `feature/sponsored-transactions` | (pendente teste real com Nightly) | Fallback implementado só se necessário | NOT STARTED — decisão adiada até o teste real com a extensão; o mecanismo já provado funcionar via wallet-adapter genérico reduz bastante o risco de precisar dele |

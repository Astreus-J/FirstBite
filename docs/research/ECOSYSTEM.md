...
- Existe também **sCOOK**: descrito como a representação do COOK no lado Solana, usada para bridging 1:1 via Hyperlane ("You acquire sCOOK on Solana e bridge it 1:1... via hyperlane.cookiescan.io"). **Atualizado (2026-09-14, verificação posterior)**: **FATO CONFIRMADO via WebSearch síntese de docs.cookiechain.wtf** — "COOK on Solana is a Token-2022 mint". O processo prático documentado é: comprar SOL → swap SOL→COOK no jup.ag → bridge para Cookie Chain via `hyperlane.cookiescan.io`. Isso resolve a lacuna anterior (SPL clássico vs Token-2022): é **Token-2022**.
...
2. ~~NOT VERIFIED: padrão exato do token sCOOK na Solana~~ — **RESOLVIDO (2026-09-14): é Token-2022**, confirmado via docs.cookiechain.wtf.
3. **FATO CONFIRMADO (2026-09-14, verificação direta na doc oficial)**: **não existe devnet nem testnet da Cookie Chain — é mainnet-only.** Isso fecha em definitivo a dúvida sobre o requisito do bounty ("mainnet or a clearly documented testnet deployment"): para a Cookie Chain especificamente, só existe a opção mainnet. Qualquer deploy real do FirstBite precisa de COOK real na mainnet, sem atalho de rede de teste gratuita.
4. **NOT VERIFIED**: URL canônica do bridge (`bridge.cookiescan.io` vs `hyperlane.cookiescan.io` vs `hyperlane.cookiechain.wtf` — três variantes apareceram em fontes diferentes).
5. **NOT VERIFIED**: genesis hash oficial (só temos o valor reportado por uma submissão de terceiro, não por doc do sponsor) — **mas confirmado de forma independente por chamada RPC direta durante o Technical PoC** (ver `poc/RESULTS.md`), então na prática já está validado, mesmo sem confirmação textual do sponsor.
6. ~~NOT VERIFIED: divergência do "$COOKCA / contract address / Jupiter"~~ — **RESOLVIDO (2026-09-14): é real e oficial**, confirmado por WebFetch direto na homepage: "$COOKCA" é apresentado como o método primário oficial de aquisição de COOK via Jupiter. Contrato: `36ZrtQoab5MhhySaP1YSTwUahSk6GRVUTtZ6cuVfm9e1`. **Atenção**: liquidez muito baixa observada nos pools (Dexscreener, 2026-09-14: ~$23k no melhor pool, ~$2 em outro) — comprar exige tolerância a slippage.
7. **Confirmado, mas com fricção conhecida**: Nightly não publica Cookie Chain no Wallet Standard, então avisos de "rede errada" podem aparecer mesmo em transações corretas — isso deve ser tratado explicitamente na UX de claim do FirstBite (ex.: aviso próprio explicando isso, para não assustar o usuário novo).
8. **Confirmado**: não existe faucet oficial da chain. Qualquer sponsorship de fee hoje é ad-hoc, mantido por cada app individualmente (ex.: "operator demo wallet" com gas drip pontual do sponsor). Isso significa que o FirstBite precisaria manter sua própria carteira "relayer/sponsor" com saldo de COOK — não há um serviço de paymaster genérico oficial da Cookie Chain (diferente do que existe oficialmente no ecossistema Solana mainnet via Solana Cookbook "Fee Sponsorship" e produtos como Blink Labs "Gas Sponsorship").
